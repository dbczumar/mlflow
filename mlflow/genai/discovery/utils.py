...
def _run_deep_analysis(
    ...
    def _analyze(idx: int, session_id: str, session_traces: list[Trace]) -> _ConversationAnalysis:
        t0 = time.time()
        preamble, conv_text = _build_conversation_for_analysis(
            session_traces, rationale_map
        )
        failing_trace_ids = [
        ...
        t1 = time.time()
        result = _run_deep_analysis_single(
            preamble, conv_text, analysis_model, failing_trace_ids
        )
        _logger.info(
...
def _cluster_by_llm(
    ...
    groups = result if isinstance(result, list) else result.get(
    ...
    result = get_chat_completions_with_structured_output(
        ...
        messages=[
            ...
            ChatMessage(
                role="user",
                content=(
                    f"Cluster of {len(cluster_indices)} analyses:\n\n"
                    f"{analyses_text}"
                ),
            ),
        ],
...
def _generate_scorer_specs(
    ...
    examples_text = "\n".join(
        f"- [surface: {a.surface}] {a.symptoms} "
        f"(root cause: {a.root_cause[:200]})"
