...
class JudgeToolRegistry:
    ...
    def invoke(self, tool_call: Any) -> Any:
        """
        Invoke a tool using a ToolCall instance and trace ID.

        Args:
            tool_call: The ToolCall containing function name and arguments.
                Must includes trace_id to invoke the tool with.
...
@experimental(version="3.4.0")
def invoke_judge_tool(tool_call: Any, trace_id: str) -> Any:
    """
    Invoke a judge tool using a ToolCall instance and trace ID.
    ...
    Returns:
        The result of the tool execution
    """
    return _judge_tool_registry.invoke(tool_call, trace_id)
