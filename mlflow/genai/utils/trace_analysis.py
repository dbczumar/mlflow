...
class IssueDescription(pydantic.BaseModel):
    ...
    context: str = pydantic.Field(
        description=(
            "Context explaining what the AGENT was asked to do or what task/scenario the agent "
            "was handling. This MUST describe the agent's task, request, or operational scenario "
            "- NOT system architecture, instructions, processing steps, or internal component "
            "behavior. "
            "\n\n"
            "GOOD examples (describe what agent was asked to do): "
            "'Agent was asked about S&P 500 stock status', "
            "'Agent was asked to provide holiday schedule information via voice', "
            "'Agent was asked to analyze documents about HR policies', "
            "'Agent received an emotionally charged voice command to delete something'. "
            "\n\n"
            "BAD examples (describe system environment/processing - NEVER USE): "
            "'Agent operates under system instructions emphasizing fast responses', "
...
def _find_issues_in_session(
    ...
    system_content += (
        ...
        "handling it'. "
        "(c) If Issue B describes HOW or WHY Issue A occurred (the mechanism/cause), they are "
        "NOT independent - B is a root cause of A, not a separate issue. TEST: 'Would fixing "
        "Issue B automatically fix/prevent Issue A?' If YES, they're the same issue with B as "
        "a root cause. "
        "Examples of INDEPENDENCE: "
        "GOOD (truly independent): 'Agent provided incorrect stock data' + 'Agent took too long "
        "to respond' (two distinct user-facing problems). "
        "BAD (mechanism, not independent): 'Agent gave incomplete response' + 'Agent didn't use "
        "tools to gather info' (not using tools is WHY response was incomplete - same issue!). "
        "BAD (mechanism, not independent): 'Agent answered incorrectly' + 'Tool returned wrong "
        "data' (bad tool data is WHY answer was wrong - same issue!). "
        "BAD (non-independent): 'Tool missing metadata' + 'Agent didn't compensate for missing "
        "metadata' (both contribute to same outcome). "
        "GOOD (merged correctly): 'Agent provided incomplete response due to not using available "
        "tools to gather additional information'. "
        "Before responding, you MUST RIGOROUSLY VERIFY independence using the test: 'Would "
        "fixing one issue automatically fix the other?' If YES, MERGE them with the fixing "
        "as a root cause.\n"
