...
def _create_issue():
    request_message = _get_request_message(
        ...
        schema={
            ...
            "name": [_assert_required, _assert_string],
            "description": [_assert_string],
            "state": [_assert_intlike],
            "tags": [],
        },
    )

    # Convert proto state enum to string
    state = None
    if request_message.HasField("state"):
        state = IssueState.from_proto(request_message.state)

    issue = IssueEntity(
        issue_id=None,  # Will be generated by store
        experiment_id=request_message.experiment_id,
        name=request_message.name,
        description=request_message.description
        if request_message.HasField("description")
        ...
        state=state,
        tags=dict(request_message.tags) if request_message.tags else None,
    )

    created_issue = _get_tracking_store().create_issue(issue)
    response_message = CreateIssue.Response()
    response_message.issue.CopyFrom(created_issue.to_proto())
    return _wrap_response(response_message)
...
def _get_issue():
    ...
    response_message = GetIssue.Response()
    response_message.issue.CopyFrom(issue.to_proto())
    return _wrap_response(response_message)
...
def _update_issue():
    ...
    issue = _get_tracking_store().update_issue(
    ...
    response_message = UpdateIssue.Response()
    response_message.issue.CopyFrom(issue.to_proto())
    return _wrap_response(response_message)
...
def _delete_issue():
    ...
    _get_tracking_store().delete_issue(request_message.issue_id)
    response_message = DeleteIssue.Response()
    return _wrap_response(response_message)
...
def _search_issues():
    request_message = _get_request_message(
        ...
        schema={
            "experiment_id": [_assert_required, _assert_string],
            "states": [_assert_array],
            "max_results": [_assert_intlike],
            "page_token": [_assert_string],
        },
    )

    # Convert proto state enums to strings
    states = None
    if request_message.states:
        states = [IssueState.from_proto(s) for s in request_message.states]
    ...
    for issue in issues:
        response_message.issues.append(issue.to_proto())
    if issues.token:
        response_message.next_page_token = issues.token
    return _wrap_response(response_message)
...
def _link_run_to_issues():
    ...
    if issue_ids := list(request_message.issue_ids):
