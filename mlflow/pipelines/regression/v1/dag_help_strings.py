def format_help_string(help_string):
  ...
  """
  return help_string.replace("\"", "#quot;")


PIPELINE_YAML = format_help_string("pipeline.yaml is the main configuration file for the pipeline. It defines attributes for each step of the regression pipeline, such as the dataset to use (defined in the 'data' section corresponding to the 'ingest' step) and the metrics to compute during model training & evaluation (defined in the 'metrics' section, which is used by the 'train' and 'evaluate' steps).")
...
""")
TRANSFORM_USER_CODE = format_help_string("""steps/transform.py defines customizable logic for transforming input data during model inference. Transformations are specified via the via the `transformer_fn` function, an example of which is displayed below (note that a different function name or module can be specified via the 'transform_method' attribute of the 'transform' step definition in pipeline.yaml):\n\n
def transformer_fn():
    \"\"\"
    Returns an *unfit* transformer with ``fit()`` and ``transform()`` methods.
    The transformer's input and output signatures should be compatible with
