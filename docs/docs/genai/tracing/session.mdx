...
The following example shows how to use session ID in a chat model that has been implemented using the <APILink fn="mlflow.pyfunc.ChatModel" /> class. Refer to the [Setting Trace Tags](/genai/tracing/attach-tags) section for more information on how to set tags on traces.
...
Alternatively, you can use the <APILink fn="mlflow.search_traces" /> function to get these traces programmatically. Refer to [Searching and Retrieving Traces](/genai/tracing/search-traces) for more details.
